...
stateDiagram-v2
    ...
    RUNNING_TO_BASE --> [*] : 抵達基地門口 (x = 200) 安全收容 (實體銷毀，分數 +50，進度 +1)
```
...
* **安全撤離跑動 AI（RUNNING_TO_BASE）**：當人質從直升機卸載下機時，會被指派為此狀態，並以 $1.5$ 倍標準奔跑速度（每幀 2.1 像素）全力朝向位於世界座標 $x = 200$ 的基地大門奔跑。若大門在人質左側則面向左，在右側則面向右。當與大門的水平距離小於 5 像素時，即判定成功進入基地，銷毀實體並觸發分數與救援進度結算。
