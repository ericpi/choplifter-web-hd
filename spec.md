...
4. **滿載返航**：各直升機依機型有不同的最大載客人數（MD-500 為 3 人、CH-47 為 8 人、Airwolf 為 2 人）。裝載後返回基地降落。
...
| **Space (空白鍵)** | 發射武器 (Shoot) | 朝直升機當前面對的方向發射雷射子彈（冷卻時間為 200 毫秒）。 |
...
| 機型名稱 | 內部標識 | 物理修改器屬性 | 外觀設計與動畫特徵 |
| :--- | :--- | :--- | :--- |
| **MD-500 Defender**<br>(美式輕型防衛者) | `BLACK_HAWK` | `gravityMod = 1.0`<br>`speedMod = 1.0`<br>`maxPassengers = 3` | 標準墨綠色軍用直升機。配有單頂部旋翼、尾翼以及底部起落架雪橇（Skids）。 |
| **CH-47 Chinook**<br>(契努克雙翼運輸機) | `CHINOOK` | `gravityMod = 0.8` (下降較慢)<br>`speedMod = 0.75` (水平略慢)<br>`maxPassengers = 8` | 雙旋翼縱列式（Tandem Rotors）長機身。配有前/後兩組旋翼同步旋轉動畫，底部起落架為輪式。 |
| **Airwolf**<br>(飛狼超音速直升機) | `AIRWOLF` | `gravityMod = 1.1` (下降偏快)<br>`speedMod = 1.41` (水平超音速)<br>`maxPassengers = 2` | 經典流線型黑白相間機身。繪製時利用 $Y$ 軸 $0.75$ 的縮放進行高度壓扁，使其顯得更加低矮修長，尾翼流線化，不顯示起落架（收回狀態）。 |

> [!NOTE]
> 經過調整後，Airwolf 展現了其標誌性的超音速性能，其水平速度修改器 `speedMod` 已大幅提升至 `1.41`，但代價是最大乘載量被壓縮至極低的 `2` 人。而 CH-47 則展現了極高的乘載力（`8` 人），但也承受著較慢的飛行速度（`speedMod = 0.75`）。
...
* **人質載運**：
  * 載客上限：依機型而定（MD-500 為 3 人、CH-47 為 8 人、Airwolf 為 2 人）。
...
* **武器發射**：按空白鍵時，若距離上一次發射大於 200 毫秒，則朝機頭朝向（朝右為 $0.1\text{ rad}$，朝左為 $\pi - 0.1\text{ rad}$）發射子彈，子彈初速會疊加直升機當前的水平速度 $vx$，並觸發持續 0.05 秒的槍口閃光（Muzzle Flash）。
...
* **移動動畫**：處於 `FOLLOWING` 狀態時，人質會以每幀 2 像素的速度朝直升機奔跑，並以正弦函數 `Math.sin(timer * 15) * 5` 來回擺動雙腿，呈現跑步效果。
...
### 7.5 Projectile (子彈)
* **速度**：基礎飛行速度為每幀 15 像素（方向由發射角度決定）。
* **碰撞偵測 (AABB Collision)**：
  子彈會與敵對陣營的實體進行碰撞檢測（玩家子彈與友軍直升機、敵方坦克子彈與敵方坦克之間不產生判定）：
  * 碰撞邊界判定：若子彈中心坐標位於目標實體的寬度與高度矩形包圍盒內，即判定為命中。
  * 命中結果：
    * 坦克被玩家子彈擊中：坦克標記刪除，加 100 分，觸發「大爆炸（BIG）」特效。
    * 直升機被坦克子彈擊中：直升機標記刪除，扣除 1 條生命，觸發「大爆炸（BIG）」特效。
* **生命期與邊界銷毀**：子彈最大生存時間為 2.0 秒，或當子彈 Y 坐標觸及地面（$y > 980$）時，會主動自我銷毀。
...
* **浮動文字**：
  在特定事件發生時（如人質登機 `"SAVED!"`、滿載 `"FULL!"`、後援 `"REINFORCEMENTS DETECTED!"`），在指定坐標生成文字，並在 1.5 秒的生命週期內不斷向上飄移（$vy = -1$）且線性淡出。
...
| 人質乘載上限 | 依機型而定 (2/3/8) | 每台直升機最多能同時搭載的人質數量 |
| 人質過關目標數 | `6` | 每一關必須送回基地的人質基準數 |
| 槍枝射擊冷卻時間| `200ms` | 限制玩家連續按空白鍵發射子彈的最小間隔 |
